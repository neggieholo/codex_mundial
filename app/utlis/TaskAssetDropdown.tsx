...
export default function TaskAssetDropdown({ assets, formData, setFormData }: Props) {
    ...
    return (
        <div className="relative">
            ...
            >
                {formData.asset_id
                    ? assets.find((a) => a.id === formData.asset_id)?.name
                    : "Select an asset"}
            </button>
                ...
                <div className="absolute z-10 mt-2 w-full bg-base-100 shadow-lg rounded-lg p-2 max-h-64 overflow-y-auto">
                    ...
                    {filtered.map((a) => (
                        <div
                            key={a.id}
                            onClick={() => {
                                setFormData((prev) => ({ ...prev, asset_id: a.id }));
                                setIsOpen(false);
                            }}
